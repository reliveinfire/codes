...
class Solution {
        public:
        int maxVacation(vector<vector<int>>& flight, vector<vector<int>>& vocation){
		// dp[week][city]
		vector<vector<int>> dp(vocation[0].size(), vector<int>(vocation.size(),0));
		vector<unordered_set<int>> pre(2);
		int cur = 0;
		int ans = 0;
			...
			for (int j = 0 ; j < dp[0].size() ; j++){
				if (i==0){
					if (flight[0][j] || j==i){
						...
						if (pre[cur^1].count(j)!=0){
							dp[i][j] = dp[i-1][j] + vocation[j][i];
							pre[cur].insert(j);
						}
						for (int k = 0 ; k < dp[0].size() ; k++){
						if (flight[k][j] && pre[cur^1].count(k)!=0){
							dp[i][j] = max(dp[i][j], dp[i-1][k] + vocation[j][i]);
							pre[cur].insert(j);
						}
					}
				}
...
class Solution {
public:
    int maxVacation(vector<vector<int>>& flights, vector<vector<int>>& days) {
...
{
#if 0
	vector<vector<int>> flight = {{0,1,1},
				      {1,0,1},
				      {1,1,0}};
	vector<vector<int>> vocation = {{1,3,1},{6,0,3},{3,3,3}};
#elif 1
	vector<vector<int>> flight = {{0,0,0},
				      {0,0,0},
				      {0,0,0}};
	vector<vector<int>> vocation = {{1,1,1},{7,7,7},{7,7,7}};
#else
	vector<vector<int>> flight = {{0,1,1},
				      {1,0,1},
				      {1,1,0}};
	vector<vector<int>> vocation = {{7,0,0},{0,7,0},{0,0,7}};
	
#endif
	Solution sol;
	int ret;
	ret = sol.maxVacation(flight, vocation);
	cout << ret << "\n";
