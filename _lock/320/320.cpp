...
class Solution {
        public:
        vector<string> wordabb(string s){
		vector<string> ans;
		helper(s,0, "", 0, ans);
		return ans;
        }
	void helper(string s, int cur, string cs, int cnt, vector<string>& ans){
		if (cur == s.size()){
			if (cnt!=0)
				cs += to_string(cnt);
			ans.push_back(cs);
		...
		helper(s, cur + 1, cs, cnt+1,ans);
		if (cnt>0)
			helper(s, cur + 1 , cs + to_string(cnt) + s[cur], 0, ans);
		else
			helper(s, cur + 1, cs + s[cur], 0, ans);
		return;
	}
